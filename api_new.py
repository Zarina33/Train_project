...
# Маршрут для получения информации о вагоне и изображений
@app.route('/api/get_train_image', methods=['POST'])
def get_wagon_images():
    ...
    try:
        # Запрос к базе данных для получения деталей вагонов
        wagon_details = TrainDetail.objects.filter(serial_number=train_number, train__date=date)

        if not wagon_details.exists():
            return jsonify({"error": "Вагон с указанным номером и датой не найден."}), 404

        images_base64 = []
        for detail in wagon_details:
            image_path = detail.image_link
            ...
            if image_base64:
                ...
                logging.warning(f"Не удалось обработать изображение по пути: {image_path}")

        if not images_base64:
            return jsonify({"error": "Невозможно получить изображения."}), 500

        logging.info(f"Изображения успешно получены и закодированы для номера поезда: {train_number}")
        return jsonify({
            "serial_number": train_number,
            "date": date_str,
            "train_images_base64": images_base64
        }), 200
